...
class CryptoConverter:
    ...
    def convert(quote: str, base: str, amount: str):
        ...
        if quote == base:
            raise ConvertionException(f"You don't need to convert {quote} in {quote}, don't you?")

        try:
            quote_ticker = keys[quote]
        except KeyError:
            raise ConvertionException(f"It was not possible to process currency {quote}")

        try:
            base_ticker = keys[base]
        except KeyError:
            raise ConvertionException(f"It was not possible to process currency {base}")

        try:
            amount = float(amount)
        except ValueError:
            raise ConvertionException(f'{amount} could not be processed')
